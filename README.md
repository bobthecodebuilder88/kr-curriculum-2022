...
교육부·한국교육과정평가원이 운영하는 **[KICE 학생평가지원포털](https://stas.moe.go.kr/cmn/main)**
에서 성취기준을 직접 조회하실 수 있습니다. **로그인 없이** 메인 화면의 `성취기준` 탭에서
학교급 → 학년군 → 교과 → 과목을 고르시면 됩니다.

이 저장소가 `needs_review`나 `출처 불일치`로 표시한 자리, 문장을 싣지 못한 39건,
번호가 끊긴 26개를 만나셨다면 **여기서 확인하시는 것이 가장 빠릅니다.**
포털은 PDF를 거치지 않은 원본 텍스트라 이 저장소가 겪은 변환 손상이 없고,
성취기준별 **해설·평가기준·영역(단원)** 정보도 함께 제공합니다.

> 참고로 이 저장소는 중학교 수학 60건을 포털과 대조해 **전부 일치**하는 것을 확인했습니다.
> 그래도 개별 인용을 공문서에 옮기실 때는 포털이나 고시 원문을 한 번 더 보시길 권합니다.
...
**여기 없는 것을 찾으신다면** [KICE 학생평가지원포털](https://stas.moe.go.kr/cmn/main)에서
조회해 보시기 바랍니다. 위에 적은 계열 전문 교과와 교양 교과 상당수가 그곳에는 있습니다.
2015 개정·2009 개정 성취기준도 함께 제공하므로, 이전 교육과정 자료를 다루실 때도 쓰실 수 있습니다.
