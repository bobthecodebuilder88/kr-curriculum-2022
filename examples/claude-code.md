...
`verify.py`가 exit 1이면 Claude는 문제를 고치고 다시 검증한다. 검사 대상은 진술문만이
아니다 — 3번에서 넣은 성취수준도 서술문을 고쳐 쓰면 `LVLDIFF`로, 그 성취기준에 수록되지
않은 등급을 인용하면 `LVLMISS`로 잡힌다. 즉 3번은 부탁이 아니라 검사받는 항목이다.
